...
public class Main2 {
	...
	static class Packing {
		...
		public int pack(int capacity, int item, int satisfied,
				List<String> names) {
			System.out.println(String.format("Capacity %d, Item: %d, satisfied: %d, items: %s", capacity, item, satisfied, names.toString()));
			if (item == n) {
			...
			if (cache[capacity][item] != -1) {
				System.out.println(String.format("Cache hit: %d, %d", capacity, item));
			...
			if (capacity >= volume[item]) {
				...
				pick = pack(capacity - volume[item], item + 1, satisfied
						+ need[item], newItems);
			}
			...
			System.out.println(String.format("Push cache %d, %d, %d", capacity, item, max));
	...
	public static void main(String[] args) throws IOException {
		...
		Scanner sc = new Scanner(
				new FileInputStream(
						"/Users/worms/git/apss/apss/src/chapter9/question2/packing/input.txt"));
